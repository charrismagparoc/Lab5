...
export function useLocalData() {
  ...
  const updateIncident = useCallback(async (id, data, userName = 'System') => {
    const { id: _id, created_at, createdat, updated_at, updatedat, date_reported, dateReported, ...safe } = data
    const { data: record, error } = await supabase.from('incidents').update(safe).eq('id', id).select().single()
    if (error) throw error
  ...
  const updateResource = useCallback(async (id, data, userName = 'System') => {
    ...
    const { id: _id, created_at, createdat, updated_at, updatedat, ...safe } = data
    const { data: record, error } = await supabase.from('resources').update(safe).eq('id', id).select().single()
    if (error) throw error
  ...
  const updateUser = useCallback(async (id, data, userName = 'System') => {
    const { id: _id, created_at, createdat, updated_at, updatedat, last_login, lastLogin, ...safe } = data
    const { data: record, error } = await supabase.from('users').update(safe).eq('id', id).select().single()
    if (error) throw error
